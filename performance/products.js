...
export function setup() {
  ...
  check(response, { 'login de produtos retornou 200': (result) => result.status === 200 });
...
export default function (token) {
  const response = http.post(baseUrl, JSON.stringify({
    query: `query Products {
      products(take: 20) { id name itemPrice }
    }`,
  }), { headers: { ...headers, Authorization: `Bearer ${token}` } });

  check(response, {
    'produtos retornou 200': (result) => result.status === 200,
    'produtos sem erros GraphQL': (result) => !result.json('errors'),
  });
}